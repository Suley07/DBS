  ...
  return (
    <main className="pt-24">
      <section className="relative h-[55vh] min-h-[420px] flex items-end overflow-hidden">
        ...
        <div className="container px-4 relative z-10 pb-12">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <h1 className="text-3xl sm:text-5xl lg:text-6xl font-extrabold tracking-tight text-foreground">{service.title}</h1>
            <p className="text-lg text-foreground/75 mt-3 max-w-2xl">{service.short}</p>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container px-4">
          <div className="grid lg:grid-cols-2 gap-16">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <p className="text-foreground/75 leading-relaxed mb-6">{service.intro}</p>
              <p className="text-foreground/75 leading-relaxed">{service.description}</p>
              <div className="mt-10">
                <Link to="/offerte" className="btn-gradient-red text-primary-foreground px-8 py-4 rounded-lg text-sm font-semibold tracking-wide uppercase inline-flex items-center transition-all hover:shadow-lg hover:shadow-primary/20">
                  Offerte Aanvragen
                </Link>
              </div>
            </motion.div>
              ...
              <div className="space-y-4">
                {service.features.map((feature) => (
                  <div key={feature} className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-foreground/75 leading-relaxed">{feature}</p>
                  </div>
                ))}
              </div>
            </motion.div>
